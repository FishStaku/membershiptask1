    ...
    public function extractTo() {
      try {
        ...
        if (is_dir($this->tmp . DIRECTORY_SEPARATOR . 'db_tables')) {
          ...
          $importer = new BetterDatabaseImport($storage, $manifest->total_queries, $manifest->config->ABSPATH, $manifest->dbdomain, $this->siteurl, $this->migration);
          $importer->import();
          ...
          $this->migration->log(__('There is no Database file, omitting...', 'backup-backup'), 'INFO');
        ...
        if ($database_exist === true) {
          ...
          $tmp_file_f = ABSPATH . '/wp-config.' . $this->tmptime . '.php';
          if (file_exists($tmp_file_f)) {
            rename($tmp_file_f, ABSPATH . '/wp-config.php');
            @unlink($tmp_file_f);
          }
        ...
        $this->migration->log(__('Restore process took: ', 'backup-backup') . (microtime(true) - $start) . 's', 'INFO');
